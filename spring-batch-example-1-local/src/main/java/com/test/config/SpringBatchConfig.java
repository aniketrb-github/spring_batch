...
public class SpringBatchConfig {
	...
	public Job job(JobBuilderFactory jobBuilderFactory, StepBuilderFactory stepBuilderFactory,
		...
		Step step = stepBuilderFactory.get("ETL-file-load")
				.<User, User>chunk(100)		// batch of 100
				.reader(itemReader)
				.processor(itemProcessor)
				.writer(itemWriter)
				.build();
		
		Job job = jobBuilderFactory.get("ETL-Load")		// Job Name
		.incrementer(new RunIdIncrementer()) 	// Sequence of ID's that will be assigned to the Job Runs
		.start(step)
		.build();
